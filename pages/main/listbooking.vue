    ...
    <main v-if="!isFetching">
      <div class="">
        ...
        <div class="flex space-x-4 items-center ml-auto">
          ...
          <h5 class="card-title text-black text-lg">Page:{{ page }}</h5>
        </div>
        <div class="mt-4">
          <div
            class="flex flex-col p-4 shadow-md rounded-md border-2 border-gray-400"
          >
            <div class="overflow-x-auto sm:-mx-6 lg:-mx-8">
              <div class="inline-block min-w-full py-2 sm:px-6 lg:px-8">
                <div class="overflow-hidden">
                  <table
                    class="min-w-full text-center text-sm font-light border border-gray-400 rounded-md"
                  >
                    ...
                    <tbody>
                      ...
                      <template v-if="sortedItems && sortedItems.length > 0">
                        <tr
                          class="border-b-2 border-gray-400 hover:bg-blue-200 cursor-pointer"
                          v-for="(booking, index) in sortedItems"
                        >
                          ...
                          <td
                            @click="view(booking)"
                            class="b px-6 py-4 dark:border-white/10"
                          >
                            {{ booking.employeeId }}
                          </td>
                          <td
                          ...
                          >
                            ...
                            <span
                              v-if="booking.statusName == 'Approved'"
                              class="bg-green-500 text-white px-2 py-1 rounded-full text-sm font-medium border-2 border-gray-300"
                              >{{ booking.statusName }}</span
                            >
                            <span
                              v-if="booking.statusName == 'Pending'"
                              class="bg-yellow-500 text-white p-2 rounded-full text-sm font-medium border-2 border-gray-300"
                              >{{ booking.statusName }}</span
                            >

                            <span
                              v-if="booking.statusName == 'Disapproved'"
                              class="bg-red-500 text-white p-2 rounded-full text-sm font-medium border-2 border-gray-300"
                              >{{ booking.statusName }}</span
                            >
                            <span
                              v-if="booking.statusName == 'CheckedIn'"
                              class="bg-blue-500 text-white p-2 rounded-full text-sm font-medium border-2 border-gray-300"
                              >{{ booking.statusName }}</span
                            >
                            <span
                              v-if="booking.statusName == 'CheckedOut'"
                              class="bg-orange-500 text-white p-2 rounded-full text-sm font-medium border-2 border-gray-300"
                              >{{ booking.statusName }}</span
                            >
                            <span
                              v-if="booking.statusName == 'Cancelled'"
                              class="bg-white text-black p-2 rounded-full text-sm font-medium border-2 border-gray-300"
                              >{{ booking.statusName }}</span
                            >
                            <span
                              v-if="booking.statusName == 'Dropped'"
                              class="bg-black text-white p-2 rounded-full text-sm font-medium border-2 border-gray-300"
                              >{{ booking.statusName }}</span
                            >
                          </td>
                          ...
                          >
                            <button
                              v-if="
                                approverDetails.role == 'Approver' &&
                                booking.statusName != 'Approved'
                              "
                              class="text-white bg-green-700 hover:bg-green-800 focus:outline-none focus:ring-4 focus:ring-blue-300 font-medium rounded-full text-sm px-5 py-2.5 text-center mr-2 mb-2 0"
                            ...
                            <button
                              v-if="
                                ...
                                (booking.statusName === 'Pending' ||
                                  booking.statusName === 'Approved')
                              "
                              class="cursor-pointer text-white bg-red-700 hover:bg-red-800 focus:outline-none focus:ring-4 focus:ring-blue-300 font-medium rounded-full text-sm px-4 py-2.5 text-center mr-2 mb-2 0"
                ...
                <div class="flex items-center justify-between">
                  <span class="small mr-3"
                    ...
                    of
                    <span id="totalpages"
                      >{{ bookingdata.totalpages }} Entries</span
                    ></span
                  >
                  ...
                  >
                    <ul class="inline-flex items-center -space-x-px">
                      <Paginate
                        v-if="bookingdata.totalpages > 1"
                        class=""
                        v-model="paginate"
                        :page-count="bookingdata.totalpages"
...
onMounted(() => {
  ...
  // if (approverDetails.role == "User is not an approver") {
  //   // element.isHidden = !(approverDetails == "Approver");
  //   tableHeader.splice(8, 1);
  // }
});
...
function assignRoom(data) {
  router.push(`/main/assignDorm?Id=${data.id}`);
}

function view(data) {
  router.push(`/main/viewBooking?Id=${data.id}`);
}
...
function goToPage(numpage: any) {
  ...
  const apiEndpoint =
    approverDetails.role === "Approver"
      ? `ListofTransactions/ListOfApproverTransaction?Page=${page}&ItemsPerPage=${defaultentry}&empid=${details.empid}&dormId=${approverDetails.dormitoryId}`
      : `ListofTransactions/ListOfTransaction?Page=${page}&ItemsPerPage=${defaultentry}&empid=${details.empid}`;
...
function search($event: { target: { value: string } }) {
  ...
  const apiEndpoint =
    approverDetails.role === "Approver"
      ? `ListofTransactions/ListOfApproverTransaction?Page=${page}&ItemsPerPage=${defaultentry}&empid=${details.empid}&dormId=${approverDetails.dormitoryId}&search=${Search.value}`
      : `ListofTransactions/ListOfTransaction?Page=${page}&ItemsPerPage=${defaultentry}&empid=${details.empid}&search=${Search.value}`;
  ...
  GetUserSubMenu(); // Assuming GetUserSubMenu() needs to be called after the API call
}
// Search Pagination//
...
function showEntries($event: { target: { value: any } }) {
  ...
  const apiEndpoint =
    approverDetails.role === "Approver"
      ? `ListofTransactions/ListOfApproverTransaction?Page=${currentPage.value}&ItemsPerPage=${defaultentry}&empid=${details.empid}&dormId=${approverDetails.dormitoryId}&search=${Search.value}`
      : `ListofTransactions/ListOfTransaction?Page=${currentPage.value}&ItemsPerPage=${defaultentry}&empid=${details.empid}&search=${Search.value}`;
  connecttoapi()
    .post(apiEndpoint)
    .then((response) => {
      if (response.data != null) {
...
function SearchEntries($event: { target: { value: any } }) {
  ...
  const apiEndpoint =
    approverDetails.role === "Approver"
      ? `ListofTransactions/ListOfApproverTransaction?Page=${paginate.value}&ItemsPerPage=${defaultentry}&empid=${details.empid}&dormId=${approverDetails.dormitoryId}&search=${Search.value}`
      : `ListofTransactions/ListOfTransaction?Page=${paginate.value}&ItemsPerPage=${defaultentry}&empid=${details.empid}&search=${Search.value}`;
  ...
  if (gotopge.value == "") {
    FirstPage(); // Assuming FirstPage() handles resetting to the first page
  }
  GetUserSubMenu(); // Assuming GetUserSubMenu() needs to be called after the API call
}
// Search Entries Pagination//
...
async function GetUserSubMenu() {
  isFetching.value = true;
  try {
    if (approverDetails.role == "Approver") {
...
function FirstPage() {
  ...
  connecttoapi()
    .post(
      `ListofTransactions/ListOfTransaction?Page=${page}&ItemsPerPage=${defaultentry}&search=${Search.value}`
    )
    .then((response) => {
      if (response.data != null) {
        bookingdata.value = response.data;
...
const sortedItems = computed(() => {
  return bookingdata?.value?.transactionlist
    ?.slice()
...
async function cancel(booking) {
  ...
  try {
    ...
    const result = await Swal.fire({
      ...
      text: "Are you sure you want to proceed with cancelling out this booking?",
      icon: "warning",
    ...
    if (result.isConfirmed) {
      ...
      const response = await connecttoapi().post(
        `Transactions/Cancel?id=${booking.id}`,
        data
      );
