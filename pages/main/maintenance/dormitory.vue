<template>
  <div>
    ...
    <div class="mb-2 ml-10">
      <label class="font-bold text-3xl">Dormitory</label>
    </div>
              ...
              <div class="inline-block min-w-full py-2 sm:px-6 lg:px-8">
                    ...
                    <tbody>
                      ...
                      <template v-if="sortedItems && sortedItems.length > 0">
                        <tr
                          ...
                          v-for="(dormitory, index) in sortedItems"
                        >
                          ...
                          <td
                            class="whitespace-nowrap b px-6 py-4 dark:border-white/10"
                          >
                            {{ dormitory.otherInformation }}
                          </td>
                          <td
                            class="whitespace-nowrap b px-6 py-4 dark:border-white/10"
                          >
                            {{ formatDate(dormitory.createdDate) }}
                          </td>
                          <td
                            class="whitespace-nowrap b px-6 py-4 dark:border-white/10"
                          >
                            {{ dormitory.statusName }}
                          </td>
                          ...
                          >
                            ...
                            <button
                              class="space-x-2 flex items-center justify-center text-white bg-blue-700 hover:bg-blue-800 focus:outline-none focus:ring-4 focus:ring-blue-300 font-medium rounded-full text-sm px-5 py-2.5 text-center mr-2 mb-2 0"
                ...
                <div class="flex items-center justify-between">
                  <span class="small mr-3"
                    ...
                    of
                    <span id="totalpages"
                      >{{ dormitoryData.totalpages }} Entries</span
                    ></span
                  >
                  ...
                  >
                    <ul class="inline-flex items-center -space-x-px">
                      <Paginate
                        v-if="dormitoryData.totalpages > 1"
                        class=""
                        v-model="paginate"
                        :page-count="dormitoryData.totalpages"
...
function manageRoom(data) {
  router.push(`/main/maintenance/manageDormitory?Id=${data.id}`);
}
...
function goToPage(numpage: any) {
  ...
  connecttoapi()
    .post(
      `ListOfDormitories/ListOfDormitory?Page=${page}&ItemsPerPage=${defaultentry}&userDormitoryId=${approverDetails.dormitoryId}`
    )
    .then((response) => {
      if (response.data != null) {
        dormitoryData.value = response.data;
        console.log(dormitoryData.value);
      }
    });
}
...
function search($event: { target: { value: string } }) {
  ...
  connecttoapi()
    .post(
      `ListOfDormitories/ListOfDormitory?Page=${page}&ItemsPerPage=${defaultentry}&search=${Search.value}&userDormitoryId=${approverDetails.dormitoryId}`
    )
    .then((response) => {
      if (response.data != null) {
        dormitoryData.value = response.data;
...
function showEntries($event: { target: { value: any } }) {
  ...
  connecttoapi()
    .post(
      `ListOfDormitories/ListOfDormitory?Page=${currentPage.value}&ItemsPerPage=${defaultentry}&search=${Search.value}&userDormitoryId=${approverDetails.dormitoryId}`
    )
    .then((response) => {
      if (response.data != null) {
        dormitoryData.value = response.data;
...
function SearchEntries($event: { target: { value: any } }) {
  ...
  connecttoapi()
    .post(
      `ListOfDormitories/ListOfDormitory?Page=${paginate.value}&ItemsPerPage=${defaultentry}&search=${Search.value}&userDormitoryId=${approverDetails.dormitoryId}`
    )
    .then((response) => {
      if (response.data != null) {
        dormitoryData.value = response.data;
...
const sortedItems = computed(() => {
  return dormitoryData?.value?.dormlist
    ?.slice()
