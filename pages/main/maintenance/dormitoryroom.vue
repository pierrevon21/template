<template>
  <div>
    ...
    <div class="mb-2 ml-10">
      <label class="font-bold text-3xl">Dormitory Room</label>
    </div>
              ...
              <div class="inline-block min-w-full py-2 sm:px-6 lg:px-8">
                ...
                <div class="flex items-center justify-between">
                  <span class="small mr-3"
                    ...
                    of
                    <span id="totalpages"
                      >{{ dormitoryRoomData.totalpages }} Entries</span
                    ></span
                  >
                  ...
                  >
                    <ul class="inline-flex items-center -space-x-px">
                      <Paginate
                        v-if="dormitoryRoomData.totalpages > 1"
                        class=""
                        v-model="paginate"
                        :page-count="dormitoryRoomData.totalpages"
                    ...
                    <ul class="pagination flex items-center justify-between">
                      ...
                      <li class="page-item">
                        <input
                          ...
                          value="1"
                          min="1"
                          :max="dormitoryRoomData.totalpages"
                          class="border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5"
                          style="width: 60px"
...
function goToPage(numpage: any) {
  ...
  connecttoapi()
    .post(
      `ListOfDormitoryRooms/ListOfDormitoryRoom?Page=${page}&ItemsPerPage=${defaultentry}`
    )
    .then((response) => {
      if (response.data != null) {
        dormitoryRoomData.value = response.data;
        console.log(dormitoryRoomData.value);
      }
    });
}
...
function search($event: { target: { value: string } }) {
  ...
  connecttoapi()
    .post(
      `ListOfDormitoryRooms/ListOfDormitoryRoom?Page=${page}&ItemsPerPage=${defaultentry}&search=${Search.value}`
    )
    .then((response) => {
      if (response.data != null) {
        dormitoryRoomData.value = response.data;
...
function showEntries($event: { target: { value: any } }) {
  ...
  connecttoapi()
    .post(
      `ListOfDormitoryRooms/ListOfDormitoryRoom?Page=${currentPage.value}&ItemsPerPage=${defaultentry}&search=${Search.value}`
    )
    .then((response) => {
      if (response.data != null) {
        dormitoryRoomData.value = response.data;
...
function SearchEntries($event: { target: { value: any } }) {
  ...
  connecttoapi()
    .post(
      `ListOfDormitoryRooms/ListOfDormitoryRoom?Page=${paginate.value}&ItemsPerPage=${defaultentry}&search=${Search.value}`
    )
    .then((response) => {
      if (response.data != null) {
        dormitoryRoomData.value = response.data;
...
const sortedItems = computed(() => {
  return dormitoryRoomData?.value?.dormroomlist
    ?.slice()
